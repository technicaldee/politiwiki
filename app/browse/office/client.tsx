...
interface BrowseByOfficeClientProps {
  politicians: Politician[]
}

export function BrowseByOfficeClient({ politicians }: BrowseByOfficeClientProps) {
  // Group politicians by office
  const politiciansByOffice = politicians.reduce((acc, politician) => {
    const office = politician.office || 'Other'
    if (!acc[office]) {
      acc[office] = []
    }
    acc[office].push(politician)
    return acc
  }, {} as Record<string, Politician[]>)

  return (
    <div className="container py-12">
      <h1 className="text-3xl font-bold mb-8">Browse Politicians by Office</h1>
      <SearchPoliticians politicians={politicians} />
      <div className="space-y-8">
        {Object.entries(politiciansByOffice).map(([office, politicians]) => (
          <div key={office}>
